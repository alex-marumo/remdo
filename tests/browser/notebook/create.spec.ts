...
test("add the first child to note with existing children", async ({ notebook, page }) => {
  ...
  await expect(page.locator('.editor-input ul').first().locator('> li')).toHaveCount(2);
  ...
  // Verify child added: note0 has a nested <ul> with one more child
  await expect(page.locator('.editor-input ul > li:has(span[data-lexical-text="true"]:text-is("note0")) > ul > li')).toHaveCount(2); // Assuming note00 exists
  await expect(page.locator('.editor-input ul').first().locator('> li')).toHaveCount(2);
  await expect(page.locator('.editor-input ul > li span[data-lexical-text="true"]:text-is("note0")')).toHaveCount(1);
...
test("create some empty notes", async ({ page, notebook }) => {
  ...
  await expect(page.locator('.editor-input ul').first().locator('> li')).toHaveCount(2);
  await expect(page.locator('.editor-input ul > li span[data-lexical-text="true"]')).toHaveText(['note0', 'note1']);
  expect(await notebook.getNotes()).toEqual(['note0', 'note1']);
  ...
  await expect(page.locator('.editor-input ul').first().locator('> li')).toHaveCount(4);
  await expect(page.locator('.editor-input ul > li span[data-lexical-text="true"]')).toHaveText(['note0', 'note1', '', '']);
  expect(await notebook.getNotes()).toEqual(['note0', 'note1', '', '']);
...
test("split note", async ({ page, notebook }) => {
  ...
  await expect(page.locator('.editor-input ul').first().locator('> li')).toHaveCount(3);
  await expect(page.locator('.editor-input ul > li span[data-lexical-text="true"]')).toHaveText(['note0', 'not', 'e1']);
